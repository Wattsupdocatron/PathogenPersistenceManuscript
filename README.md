...
- For metatranscriptomics/metagenomics isolate genome mapping: process reads with bbduck as per Lappan et al., 2023 + ribodetector script (metaT samples), progress to kraken2-filter script, then use subread-align script for each group, then finish with featureCounts script to generate feature counts. Hits can be searched using eggnogmapper (http://eggnog-mapper.embl.de or https://github.com/eggnogdb/eggnog-mapper) to annotate COGs, then COG group counts and TPM can be calculated and summed in Microsoft excel.
